...
import { useParams } from 'react-router-dom';
import '../styles/Notes.css';
import { FaEdit, FaTrash, FaPlus, FaCamera, FaTimes, FaVolumeUp } from 'react-icons/fa';
        ...
        <ol>
          {notes.map((note) => (
            <li key={note.id} className="note-item">
              ...
              <div className="note-actions">
                <button className="speak-btn" onClick={() => handleSpeak(note.content)} title="Read Note">
                  <FaVolumeUp /> Speak
                </button>
                <button className="edit-btn" onClick={() => handleEditClick(note)}>
                  <FaEdit /> Edit
